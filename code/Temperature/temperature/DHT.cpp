...
byte DHT::read(byte data[DHT::DATA_LENGTH])
{
  ...
  volatile uint32_t* modeRegister = portModeRegister(port);
  volatile uint32_t* outRegister = portOutputRegister(port);
  volatile uint32_t* inRegister = portInputRegister(port);
